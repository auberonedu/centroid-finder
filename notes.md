...
-----------------
wave5 - BinarizingImageGroupFinder
-> converts color image to binary array using ImageBinarizer;
-> finds connected white pixel groups using BinaryGroupFinder;
Unit testing will be generated by AI and should include mocks and fakes.
Fake: A lightweight test version of a real class with simplified logic (use when you want predictable but simple output);
Mock: A controlled test object that tracks method calls and returns what we tell it (use to verify interactions between objects);
Eg's for wave5: we can consider a unit test FakeBinarizer that returns a known 2D binary array; or a FakeGroupFinder that returns a known list of Group objects; (Then we check if findConnectedGroups() returns the expected list, or if this class passes the expected inputs from one component to the next);
-----------------
wave5 pseudocode based on class Javadoc-
//findConnectedGroups(BufferedImage image)
//convert image to binary, then find connected 1's groups
//return those groups
Check if image is null -> throw NPE
Call binarizer.toBinaryArray(image)->returns int[][] binary
Call groupFinder.findGroups(binary)->returns List<Group>
Return List.
-----------------
