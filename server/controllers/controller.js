...
const postVideo = (req, res) => {
    ...
    try {
        ...
        const RESULTS_DIR = path.resolve(process.env.RESULTS_DIR, `${jobId}.csv`); // Path to where the DSV output will be saved

        // DEV CHECK TO SEE IF jarPath EXISTS
        if (existsSync(path.resolve(process.env.JAVA_JAR_PATH))) {
